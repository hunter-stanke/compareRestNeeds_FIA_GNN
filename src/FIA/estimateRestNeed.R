...
estimate_rest_needs <- function(dirResults = here::here('results/FIA/'),
  ...
  processStrata(dirResults = dirResults,
                dat = read.csv(paste0(dirResults, '/sclass/ORWA_EW_BPS_annual.csv')),
                refcon = refCon,
                rules = rules,
                sclass = sclass,
                bps = BpS_Code,
                prefix = 'ORWA_EW_BPS_annual',
                cores = cores,
                YEAR, ew, BpS_Code)
  
  ## TI BPS x EW 
  processStrata(dirResults = dirResults,
                dat = read.csv(paste0(dirResults, '/sclass/ORWA_EW_BPS_ti.csv')),
                refcon = refCon,
                rules = rules,
                sclass = sclass,
                bps = BpS_Code,
                prefix = 'ORWA_EW_BPS_ti',
                cores = cores,
                YEAR, ew, BpS_Code)
...
processStrata <- function(dirResults, dat, refcon, rules, sclass, bps, prefix, cores, ...){
  ...
  dat <- dat %>%
    ...
    tidyr::complete(tidyr::nesting(strataID, YEAR, BpS_Code), SCLASS, 
                    fill = list(AREA_TOTAL=0, PERC_AREA=0)) %>%
