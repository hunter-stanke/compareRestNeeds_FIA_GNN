...
dat <- read.csv(here('EWA/results/FIA/sclassChng/EWA_comp.csv')) %>% 
  filter(YEAR == 2017) %>%
  select(-c(AREA_DOMAIN1, AREA_DOMAIN2, STATUS1, STATUS2))
net <- read.csv(here('EWA/results/FIA/sclassChng/EWA_net.csv')) %>% 
  filter(YEAR == 2017) %>%
...
mat
ggsave(mat, filename = here::here('EWA/figs//EWA_sclass_chng_mtrx.png'),
       height = 6, width = 9)
