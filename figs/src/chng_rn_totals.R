...
dat <- read.csv(here('EWA/results/FIA/restNeedChng/BPS_restNeed.csv'))

## Area totals
a <- read.csv(here('EWA/results/FIA/sclassChng/EWA_net.csv')) %>%
  filter(YEAR == 2017) %>%
...
plt <- dat %>%
  ...
  geom_text(aes(x = x, y = y, label = label, col = label),
            data.frame(x = c(3 - .4, 3 + 0.1), y = 3.5, 
...
ggsave(plt, filename = here('EWA/figs/compare_rn_chng.png'),
       height = 6.5, width = 10)
